...
class AverageMeter(object):
    ...
    def update(
        self, val, n=1
    ):  # avg value over samples, and the num of samples, e.g. the avg loss over a batch and batchsize
        self.val = val
        self.sum += val * n
        self.count += n
...
class Trainer:
    ...
    def train_one_step(self, step):
        ...
        text, modality, image, mask, dataset_name, query_mask = (
            batch["text"],
            batch["modality"],
            batch["image"],
            batch["mask"],
        ...
        # # Save the image and mask and text
        # save_dir = "/home/yb107/cvit-work/cvpr2025/SAT/data/test_cases"
        # filename = os.path.basename(batch["image_path"][0])
        # # Save
        # image_save_path = os.path.join(save_dir, f"{filename}")
        # np.savez(
        #     image_save_path,
        ...
        with autocast(device_type=self.device.type):
            ...
            # print(
            #     f"Text: {len(text[0])}, {text}, Dataset: {dataset_name}, modality: {modality}, Image: {image.shape}, Query Mask: {query_mask.shape} "
            # )
            # # Text: 9 ..., Dataset: ['MR_LeftAtrium'], modality: ['mri'], Image: torch.Size([1, 3, 288, 288, 96]), Query Mask: torch.Size([1, 9])
            # print(f"Mask {mask.shape}, Unique: {mask.unique()}")
            # print(f"Queries: {queries.shape}")
            # # Mask torch.Size([1, 9, 288, 288, 96]), Unique: tensor([0., 1.], device='cuda:0')
            # # Queries: torch.Size([1, 9, 768])

            # print(f"Query Mask: {query_mask}")
            ...
            # forward
            logits = self.model(
                queries=queries, image_input=image
            )  # bnhwd or list of bnhwd (deep supervision)

            # print(
            #     f"Logits: {len(logits)}, {logits[0].shape}, {logits[1].shape}, {logits[2].shape}, {logits[3].shape}"
            # )
            # Logits: 4, torch.Size([1, 9, 288, 288, 96]), torch.Size([1, 9, 144, 144, 48]), torch.Size([1, 9, 72, 72, 24]), torch.Size([1, 9, 36, 36, 12])
            ...
            weights = torch.Tensor(
                [1 / (2**i) for i in range(len(logits))]
            )  # 1, 1/2, 1/4, 1/8
            weights = weights / weights.sum()  # Convert to probability
            # loss of the largest output
            bp_loss, unreduced_batch_dice_loss, unreduced_batch_ce_loss = (
                segmentation_loss(
                    logits[0],
                    mask,
                    query_mask,
                    self.dice_loss,
                    self.bce_w_logits_loss,
                    weights[0],
                )
            )
            # loss of mid-stage output
            for mid_logits, mid_weight in zip(logits[1:], weights[1:]):
                mid_mask = torch.nn.functional.interpolate(
                    mask, size=mid_logits.shape[2:], mode="nearest"
                )  # HWD -> hwd # Downsample
                (
                    tmp_bp_loss,
                    tmp_unreduced_batch_dice_loss,
                    tmp_unreduced_batch_ce_loss,
                ) = segmentation_loss(
                    mid_logits,
                    mid_mask,
                    query_mask,
        ...
        with torch.no_grad():
            if is_master():
                ...
                for i, name in enumerate(dataset_name):
                    ...
                    self.dataset_dice_loss_m_dict[name].update(
                        unreduced_batch_dice_loss[i].item(), 1
                    )
                    self.dataset_ce_loss_m_dict[name].update(
                        unreduced_batch_ce_loss[i].item(), 1
                    )
                    if name not in self.sample_statistics:
                        self.sample_statistics[name] = 0
                    self.sample_statistics[name] += 1
                # overall loss
                self.dice_loss_m.update(
                    torch.mean(unreduced_batch_dice_loss).item(), batch_size
                )
                self.ce_loss_m.update(
                    torch.mean(unreduced_batch_ce_loss).item(), batch_size
                )
                ...
                if self.accumulation_steps == 0:
                    ...
                    if step % self.log_step_interval == 0:
                        ...
                        self.tb_writer.add_scalar(
                            "train_dice_loss/all_dataset", self.dice_loss_m.avg, step
                        )
                        self.tb_writer.add_scalar(
                            "train_ce_loss/all_dataset", self.ce_loss_m.avg, step
                        )
                        self.tb_writer.add_scalar("train/learning_rate", lr, step)
                        for name, meter in self.dataset_dice_loss_m_dict.items():
                            self.tb_writer.add_scalar(
                                f"train_dice_loss/{name}", meter.avg, step
                            )
                        for name, meter in self.dataset_ce_loss_m_dict.items():
                            self.tb_writer.add_scalar(
                                f"train_ce_loss/{name}", meter.avg, step
                            )
                        ...
                        with open(self.log_file, "a") as f:
                            ...
                            for name in sorted_keys:
                                ...
                                size, repeated_times = (
                                    self.trainset.get_size_and_repeat(name)
                                )
                                f.write(
                                    f" {name} {sampled_times}/{size}/{size+repeated_times} |"
                                )
                            f.write(f"\n")
                    ...
                    if step % self.save_large_interval == 0:
                        ...
                        tmp_path = os.path.join(
                            self.checkpoint_dir, f"text_encoder_step_{step}.pth"
                        )
                        torch.save(
                            {
                                "step": step,
                    ...
                    if (
                        self.save_small_interval
                        and step % self.save_small_interval == 0
                    ):
                        t0 = time.time()
                        torch.save(
                            {
                        ...
                        tmp_path = os.path.join(
                            self.checkpoint_dir, f"text_encoder_latest_step.pth"
                        )
                        torch.save(
                            {
                                "step": step,
