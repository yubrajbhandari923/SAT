...
def evaluate(
    ...
    if is_master():
        ...
        datasets_labels_metrics = (
            {}
        )  # {'COVID19':{'covid19_infection':{'dice':[0.8, 0.9, ...], ...} ...}, ...}

        # datasets --> samples --> labels --> metrics
        samples_labels_metrics = (
            {}
        )  # {'COVID19':{'0.npy':{'covid19_infection':{'dice':0.8, ...} ...}, ...} 记录每个dataset里的sample（行）

        # datsets --> labels
        datasets_labels_sets = (
            {}
        )  # {'COVID19':set('covid19_infection', ...), ...}  记录每个dataset里的label种类（列）

    # accumulate scores of each sample in each process
    results_of_samples = (
        []
    )  # each element : [dataset_name, modality, sample_id, scores_of_labels(dict), label_names]
    ...
    if resume and is_master():
        ...
        print(
            f"Load results of {len(results_of_samples)} samples, Merge into {merge_pkl}"
        )

    if not is_ensemble:
        model.eval()
    else:
        ensemble_router = model
    ...
    with torch.no_grad():
        ...
        for (
            sample
        ) in testloader:  # in evaluation/inference, a "batch" in loader is a volume
            # data loading
            ...
            if is_ensemble:
                model = ensemble_router(batched_patches[0], labels[0])
                model.eval()
            ...
            with autocast():
                ...
                for patches, y1y2_x1x2_z1z2_ls in zip(
                    ...
                    for b in range(len(y1y2_x1x2_z1z2_ls)):
                        ...
                        tmp = (
                            prediction_patch[b, :, : y2 - y1, : x2 - x1, : z2 - z1]
            ...
            for j in range(len(labels)):
                ...
                )  # {'dice':0.9, 'nsd':0.8} 每个label一个dict
            ...
            if visualization:
                ...
                for j, label in enumerate(labels):
                    ...
                    Path(f"{nib_dir}/{dataset_name}/seg_{sample_id}").mkdir(
                ...
                try:
                    image = np.load(image_path)["imgs"].astype(np.float32)
                except:
                    raise ValueError(
                        f"Cannot load image from {image_path}, please check the path and the file format."
                    )
                ...
                for j, label in enumerate(labels):
                    gt += gt_segmentation[j, :, :, :] * (
                        j + 1
                    )  # 0 --> 1 (skip background)
                    # Path(f"{nib_dir}/{dataset_name}/gt_{sample_id}").mkdir(
                    #     exist_ok=True, parents=True
                    # )
                    # # 每个label单独一个nii.gz
                    # segobj = nib.nifti2.Nifti1Image(
                    #     gt_segmentation[j, :, :, :].astype(np.float32), np.eye(4)
                    # )
                    # nib.save(
                    #     segobj,
            ...
            # accumulate
            results_of_samples.append(
                [dataset_name, modality, sample_id, scores, labels]
            )

            # save in each process regularly in case of interruption
            if len(results_of_samples) % save_interval == 0:
                with open(
                    csv_path.replace(".csv", f"_tmp_rank{dist.get_rank()}.pkl"), "wb"
                ) as f:
                    pickle.dump(results_of_samples, f)
        ...
        print(
            f"On Rank {dist.get_rank()}, each sample has {avg_patch_batch_num} batch of patches and {avg_query_batch_num} batch of queries, Data Time: {data_time}, Pred Time: {pred_time}, Dice Time: {metric_time}"
        )
        ...
        with open(
            csv_path.replace(".csv", f"_fnl_rank{dist.get_rank()}.pkl"), "wb"
        ) as f:
            pickle.dump(results_of_samples, f)
        ...
        if is_master():
            ...
            while True:
                ...
                for rank_id in range(torch.distributed.get_world_size()):
                    if not os.path.exists(
                        csv_path.replace(".csv", f"_fnl_rank{rank_id}.pkl")
                    ):  # xxx_tmp_rankx.pkl
            ...
            for dataset_name, modality, sample_id, scores, labels in results_of_samples:
                if f"{dataset_name}/{sample_id}" not in unique_set:
                    ...
                    deduplicated_results_of_samples.append(
                        [dataset_name, modality, sample_id, scores, labels]
                    )
            ...
            for (
                ...
                results_of_samples
            ):  #  [[dataset_name, modality, sample_id, scores_of_labels(dict), label_names], ...]
                ...
                for metric_dict, label in zip(scores, labels):
                    ...
                    if label not in datasets_labels_metrics[dataset_name]:
                        datasets_labels_metrics[dataset_name][label] = {
                    ...
                    samples_labels_metrics[dataset_name][sample_id][label] = {
                        k: v for k, v in metric_dict.items()
                    }
            ...
            for dataset in datasets_labels_metrics.keys():
                ...
                }  # 'TotalSegmentator(CT)': {'dice':[0.8, ...] 'nsd':[0.5, ...], ...}
                for label in datasets_labels_metrics[dataset].keys():
                    ...
                    for metric in datasets_labels_metrics[dataset][label].keys():
                        label_metric = np.average(
                            datasets_labels_metrics[dataset][label][metric]
                        )
                        avg_df[f"{dataset}, {label}"].append(
                            label_metric
                        )  # 'TotalSegmentator, Lung': [0.68, 0.72, ...] list of num_metrics
                        avg_df[dataset][metric].append(label_metric)
                avg_df[dataset] = {
                    k: np.average(v) for k, v in avg_df[dataset].items()
                }  # 'TotalSegmentator': {'dice':[0.8, ...] 'nsd':[0.5, ...], ...} --> 'TotalSegmentator': {'dice':0.x, 'nsd':0.x, ...}
                info += f"{dataset}  |  "
                ...
                for image_id, label_dict in samples_labels_metrics[dataset].items():
                    for metric in metric_df:
                        ...
                        for label in label_set:
                            score = (
                                label_dict[label][metric] if label in label_dict else -1
                            )
                            tmp.append(score)
