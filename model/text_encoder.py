...
class Text_Encoder(nn.Module):
    def __init__(
        ...
        if checkpoint:
            ...
            checkpoint["model_state_dict"] = {
                ...
                if "atlas_tower" not in k and "temperature" not in k
            }
            if partial_load:
                ...
                if is_master():
                    print(
                        "The following parameters are unexpected in query generator checkpoint:\n",
                        unexpected_state_dict,
                    )
                    print(
                        "The following parameters are missing in query generator checkpoint:\n",
                        missing_state_dict,
                    )
                    print(
                        "The following parameters have different shapes in query generator checkpoint:\n",
                        unmatchd_state_dict,
                    )
                    print(
                        "The following parameters are loaded in query generator :\n",
                        state_dict.keys(),
                    )
            else:
        ...
        for name, param in model.named_parameters():
            if (
                "encoder.layer." in name
                and int(name.split("encoder.layer.")[-1].split(".")[0])
                > open_bert_layer
            ):  # encoder.layer.11.xxx --> 11
    ...
    def forward(self, label_name, modality_name):
        ...
        if isinstance(label_name[0], list):
            ...
            modality = [
                mod for mod in modality_name for n in range(num_query)
            ]  # repeat each mod for N times -> BN
        else:
        ...
        modality_code = torch.tensor(
            [modality_code_dict[mod] for mod in modality]
        )  # bn
