...
class Maskformer(nn.Module):
    def __init__(
        ...
        self.backbone = {
            ...
            "UNET": PlainConvUNet(
                ...
                # features_per_stage=(64, 4, 4, 4, 4, 768),
                conv_op=nn.Conv3d,
                kernel_sizes=3,
                strides=(1, 2, 2, 2, 2, 2),
        ...
        self.projection_layer = {
            "SwinUNETR": nn.Sequential(
                nn.Linear(1536, 768), nn.GELU(), nn.Linear(768, query_dim), nn.GELU()
            ),
            "UNET": nn.Sequential(
                nn.Linear(1792, 768), nn.GELU(), nn.Linear(768, query_dim), nn.GELU()
            ),
            "UNET-L": nn.Sequential(
                nn.Linear(
                    3584, 1536
                ),  # 128, 128, 256, 512, 1024, 1536 --> 3584 --> 768
                nn.GELU(),
                nn.Linear(1536, query_dim),
                nn.GELU(),
            ),
            "UNET-H": nn.Sequential(
                nn.Linear(5632, 3072), nn.GELU(), nn.Linear(3072, query_dim), nn.GELU()
            ),
        }[vision_backbone]

        # positional encoding
        pos_embedding = PositionalEncoding3D(query_dim)(
            torch.zeros(
                1,
                (image_height // self.hw_patch_size),
                (image_width // self.hw_patch_size),
                (frames // self.frame_patch_size),
                query_dim,
            )
        )  # b h/p w/p d/p dim
        self.pos_embedding = rearrange(
            pos_embedding, "b h w d c -> (h w d) b c"
        )  # n b dim

        # (fused latent embeddings + pe) x query prompts
        decoder_layer = TransformerDecoderLayer(
            d_model=query_dim, nhead=8, normalize_before=True
        )
        decoder_norm = nn.LayerNorm(query_dim)
        self.transformer_decoder = TransformerDecoder(
            decoder_layer=decoder_layer, num_layers=6, norm=decoder_norm
        )
    ...
    def vision_backbone_forward(self, image_input):
        ...
        # print("==" * 20)
        latent_embedding_ls, per_pixel_embedding_ls = self.backbone(
            image_input
        )  # B Dim H/P W/P D/P
        ...
        image_embedding = rearrange(
            image_embedding, "b h w d dim -> (h w d) b dim"
        )  # (H/P W/P D/P) B Dim

        # add pe to image embedding
        pos = self.pos_embedding.to(
            latent_embedding_ls[-1].device
        )  # (H/P W/P D/P) B Dim
    ...
    def infer_forward(self, q, image_embedding, pos, per_pixel_embedding_ls):
        ...
        q = repeat(
            q, "n dim -> n b dim", b=B
        )  # N B Dim NOTE:By default, attention in torch is not batch_first
        q = self.query_proj(q)
        mask_embedding, _ = self.transformer_decoder(
            q, image_embedding, pos=pos
        )  # N B Dim
        ...
        logits = torch.einsum(
            "bchwd,bnc->bnhwd", per_pixel_embedding, mask_embedding
        )  # bnhwd
    ...
    def train_forward(self, queries, image_embedding, pos, per_pixel_embedding_ls):
        ...
        queries = rearrange(
            queries, "b n dim -> n b dim"
        )  # N B Dim NOTE:By default, attention in torch is not batch_first
        queries = self.query_proj(queries)
        mask_embedding, _ = self.transformer_decoder(
            queries, image_embedding, pos=pos
        )  # N B Dim
        mask_embedding = rearrange(mask_embedding, "n b dim -> (b n) dim")  # (B N) Dim
        # Dot product
        last_mask_embedding = self.mask_embed_proj(mask_embedding)  # 768 -> 128/64/48
        last_mask_embedding = rearrange(
            last_mask_embedding, "(b n) dim -> b n dim", b=B, n=N
        )
        per_pixel_embedding = per_pixel_embedding_ls[0]  # decoder最后一层的输出
        logits = [
            torch.einsum("bchwd,bnc->bnhwd", per_pixel_embedding, last_mask_embedding)
        ]

        # deep supervision
        if self.deep_supervision:
            for mask_embed_proj, per_pixel_embedding in zip(
                self.mid_mask_embed_proj, per_pixel_embedding_ls[1:]
            ):  # H/2 --> H/16
                mid_mask_embedding = mask_embed_proj(mask_embedding)
                mid_mask_embedding = rearrange(
                    mid_mask_embedding, "(b n) dim -> b n dim", b=B, n=N
                )
                logits.append(
                    torch.einsum(
                        "bchwd,bnc->bnhwd", per_pixel_embedding, mid_mask_embedding
                    )
                )
    ...
    def forward(self, queries, image_input, train_mode=True):
        # get vision features
        image_embedding, pos, per_pixel_embedding_ls = self.vision_backbone_forward(
            image_input
        )

        # Train Forward -----------------------------------------------------------------------
        if train_mode:
            logits = self.train_forward(
                queries, image_embedding, pos, per_pixel_embedding_ls
            )
