...
def build_maskformer(args, device, gpu_id):
    model = Maskformer(
        args.vision_backbone, args.crop_size, args.patch_size, args.deep_supervision
    )
...
def load_checkpoint(
    ...
    if partial_load:
        ...
        missing_state_dict = [
            ...
            if k not in checkpoint["model_state_dict"].keys()
        ]
        unmatchd_state_dict = [
            k
        ...
        if is_master():
            print(
                "The following parameters are unexpected in SAT checkpoint:\n",
                unexpected_state_dict,
            )
            print(
                "The following parameters are missing in SAT checkpoint:\n",
                missing_state_dict,
            )
            print(
                "The following parameters have different shapes in SAT checkpoint:\n",
                unmatchd_state_dict,
            )
...
def inherit_knowledge_encoder(knowledge_encoder_checkpoint, model, device):
    ...
    visual_encoder_state_dict = {
        k.replace("atlas_tower", "backbone"): v
        for k, v in checkpoint["model_state_dict"].items()
        if "atlas_tower.encoder" in k
    }  # encoder部分
    model_dict.update(visual_encoder_state_dict)
    proj_state_dict = {
        k.replace("atlas_tower.", ""): v
        for k, v in checkpoint["model_state_dict"].items()
        if "atlas_tower.projection_layer" in k
    }  # projection layer部分
    ...
    if is_master():
        print(
            "** CHECKPOINT ** : Inherit pretrained unet encoder from %s"
            % (knowledge_encoder_checkpoint)
        )
        print(
            "The following parameters are loaded in SAT:\n",
