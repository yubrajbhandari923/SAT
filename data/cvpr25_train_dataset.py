...
class Med_SAM_Dataset(Dataset):
    def __init__(
        ...
        for sample in lines:
            ...
            # Yubraj: They have totalseg's dataset (maybe others too) with sampling weight 2.0 (while its 1.0 for others)
            weight = weight * self.dataset_config[sample["dataset"]]["sampling_weight"]

            # repeat times for label num
            label_num = (
                len(self.text_prompts_json[sample["dataset"]]) - 1
            )  # exclude instance label
            ...
            # allow repeat
            repeat_times = round(
                size_repeat_times * query_repeat_times
            )  # e.g. 1.5 * 2.5 = 3.75 --> 4
            
            ## @Y: What the fuck are size_repeat and query_repeat_times ???
            #
            #
        ...
        if is_master():
            print(
                f"** DATASET ** {len(lines)} unique 3D samples are loaded, {count_repeat} samples are repeated"
            )
            print(f"** DATASET ** In total {len(self.datasets)} datasets.\n")
            print(
                f"** DATASET ** Size, Repeated Times and Repeat/Size Ratio for each dataset:\n"
            )
            for k, repeated_times in self.datasets_repeat_times.items():
                size = self.datasets_size[k]
                print(
                    f"{k} : (dataset_size /  repeat_times) {size}/{repeated_times} = {repeated_times/size}"
                )
    ...
    def _crop(self, image, mc_mask, is_roi_crop, label_based_crop_prob, uncenter_prob):
        ...
        if (imgh - croph) > 0 or (imgw - cropw) > 0 or (imgd - cropd) > 0:
            ...
            if (not mc_mask.any()) or (not is_roi_crop):
                ...
                # 100% roi crop
                image, y1x1z1_y2x2z2 = self._roi_crop(
                    image, mc_mask, label_based_crop_prob, uncenter_prob
                )
        else:
    ...
    def _roi_crop(self, image, mc_mask, label_based_crop_prob, uncenter_prob):
        ...
        selected_voxel = (
            voxels_foreground[0][selected_index].item(),
            voxels_foreground[1][selected_index].item(),
            voxels_foreground[2][selected_index].item(),
        )
    ...
    def _select_pos_labels(self, label_index_ls, is_pos_ls, neg_label_ratio_threshold):
        ...
        if pos_num <= self.max_queries:
            ...
            max_neg_num = int(
                neg_label_ratio_threshold * pos_num
            )  # neg label num < (pos label num) * x%
            left_pos_num = min(
                self.max_queries - pos_num, max_neg_num
            )  # neg label num < self.max_queries-pos_num
    ...
    def get_size_and_repeat(self, dataset_name):
        return (
            self.datasets_size[dataset_name],
            self.datasets_repeat_times[dataset_name],
        )

    def sc_mask_to_mc_mask(
        self, sc_mask, label_values_ls
    ):  # basically a one-hot encoding
    ...
    def select_text_prompts(self, lists):
        ...
        # Translation: For each non-empty list, generate (original index, shuffled queue)
        queues = []
        ...
        # 随机打乱队列顺序，确保均匀选择列表
        # Translation: Randomly shuffle the order of queues to ensure uniform selection of lists
        ...
        return (
            collected_elements[: self.max_queries],
            source_indices[: self.max_queries],
        )
    ...
    def load_npz_data(self, dataset_name, data_path):
        ...
        img, sc_mask, spacing = NAME2LOADER[dataset_name](
            sample_name, img, sc_mask, spacing
        )
        img = img[np.newaxis, :, :, :]  # 1 h w d

        label_2_text_prompt = self.text_prompts_json[
            dataset_name
        ]  # '1':['xxx', 'xxx', ...]
        label_values_ls = list(label_2_text_prompt.keys())
        label_values_ls = [
            int(v) for v in label_values_ls if v != "instance_label"
        ]  # [1, 2, 3, ...]
        ...
        mc_mask = self.sc_mask_to_mc_mask(
            sc_mask, label_values_ls
        )  # n h w d # mc stand for multi-channel, sc stand for single-channel

        return (
            torch.from_numpy(img.copy()),
            torch.from_numpy(mc_mask.copy()),
            text_prompt_ls,  # Text prompts for each label
        )
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                # crop image
                roi_crop_prob = self.dataset_config[dataset_name][
                    "foreground_crop_prob"
                ]
                is_roi_crop = random.random() < roi_crop_prob
                label_based_crop_prob = self.dataset_config[dataset_name][
                    "label_based_crop_prob"
                ]
                uncenter_prob = self.dataset_config[dataset_name]["uncenter_prob"]
                image, y1x1z1_y2x2z2 = self._crop(
                    image, mask, is_roi_crop, label_based_crop_prob, uncenter_prob
                )
                ...
                neg_label_ratio_threshold = self.dataset_config[dataset_name][
                    "neg_label_ratio_threshold"
                ]
                ...
                if is_roi_crop:
                    ...
                    chosen_label_index_ls, is_pos_ls = self._select_pos_labels(
                        all_label_index_ls, is_pos_in_crop, neg_label_ratio_threshold
                    )  # [label1, label2, ....], [True, False, ...]
                else:
                    chosen_label_index_ls = random.sample(
                        all_label_index_ls,
                        min(self.max_queries, len(all_label_index_ls)),
                    )
                ...
                if len(chosen_label) == self.max_queries:
                    ...
                    chosen_label, source_indices = self.select_text_prompts(
                        chosen_label
                    )
                ...
                if not isinstance(mask, torch.FloatTensor):
                    mask = mask.float()  # 转换为浮点类型 Translation: Convert to float type

                # simple check
                _, H, W, D = image.shape
                N, mH, mW, mD = mask.shape
                assert (
                    H == mH and W == mW and D == mD
                ), f"image shape {H, W, D} inconsistent with mask shape {mH, mW, mD}"
                assert N == len(
                    chosen_label
                ), f"query num {len(chosen_label)} inconsistent with gt mask channels {N}"
        ...
        # print(
        #     f"Dataset: {dataset_name}, Data Path: {data_path}, Image Shape: {image.shape}, Mask Shape: {mask.shape}, Labels: {chosen_label}"
        # )
...
if __name__ == "__main__":
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        if img_np.shape[0] == 1:
            img_np = img_np[0]
        nib.save(
            nib.Nifti1Image(img_np, affine),
            os.path.join(sample_dir, f"(img){basename}.nii.gz"),
        )

        mask_np = mask.numpy()
        results = np.zeros(
            (mask_np.shape[1], mask_np.shape[2], mask_np.shape[3])
        )  # hwd
        for j in range(mask_np.shape[0]):
            results += mask_np[j, :, :, :] * (
                j + 1
            )  # 0 --> 1 (skip background)            # 每个label单独一个nii.gz
        nib.save(
            nib.Nifti1Image(results, affine),
            os.path.join(sample_dir, f"(seg){basename}.nii.gz"),
        )
