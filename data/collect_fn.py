...
def collect_fn(data):
    ...
    # You get 1 image, one hot encoded mask, and 
    
    # pad to max depth in the batch
    # pad to max num of class in the batch
    max_class = 1
    for sample in data:
        class_num = sample["mask"].shape[0]  # So here it's one-hot encoded ?
    ...
    for i, sample in enumerate(data):
        if sample["image"].shape[0] == 1:
            sample["image"] = repeat(
                sample["image"], "c h w d -> (c r) h w d", r=3
            )  # Why r=3?

        image.append(sample["image"])

        class_num = sample["mask"].shape[0]
        pad = (0, 0, 0, 0, 0, 0, 0, max_class - class_num)  # basically pad to max_class
        ...
        # Query mask is a binary mask indicating which classes are present in the sample
        # It passes the information about what classes were padded later and what classes were already present
